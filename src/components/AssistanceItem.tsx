...
const AssistanceItem = () => {
  ...
  const { deleteAssistance, getAssistanceById } = useManageAssistancesCompaniesContext();

  const { setTotalAcceptedAssistances, setTotalNonAcceptedAssistances, setTotalAmount } = useFilter();
    ...
    <div>
      {assistancesList && assistancesList.length > 0 ? (
        assistancesList.map((assistance: AssistanceDatas, index: number) => (
          <div key={assistance.id} className={`grid grid-cols-7 content-center items-center py-2 ${index % 2 === 0 ? "bg-slate-200" : "bg-white"}`}>
            <h2 className="text-center">{formatDate(assistance.data_intervento)}</h2>
            <h2 className="text-center capitalize">{assistance.nome_compagnia}</h2>
            <h2 className="text-center">{assistance.targa}</h2>
            <h2 className="text-center">{assistance.numero_dossier}</h2>
            <h2 className="text-center">{assistance.esito_intervento ? "Sì" : "No"}</h2>
            <h2 className="text-center">{formatPriceEurCurrency(assistance.importo_intervento)}</h2>
        ...
        <Center mt="50">
          <p className="font-bold text-3xl text-red-700">Nessun intervento disponibile</p>
        </Center>
      )}
    </div>
  );
